...
#>
	...
	process
	{
		try { $uri = '{0}{1}' -f (Get-PSFConfigValue -FullName 'VHDX.Client.Uri' -NotNull), $FunctionName }
		catch { $PSCmdlet.ThrowTerminatingError($_) }
		$header = @{ }
		
		#region Authentication
		$unprotectedToken = Get-PSFConfigValue -FullName 'VHDX.Client.UnprotectedToken'
		$protectedToken = Get-PSFConfigValue -FullName 'VHDX.Client.ProtectedToken'
