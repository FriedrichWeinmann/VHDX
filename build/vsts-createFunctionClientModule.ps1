...
$parentModule = 'VHDX'
if (-not $ModuleName) { $ModuleName = 'VHDX.Client' }
...
$publishRoot = Join-Path -Path $WorkingDirectory -ChildPath 'publish\VHDX'
...
$originalManifestData = Import-PowerShellDataFile -Path "$publishRoot\VHDX.psd1"
$prereqHash = @{
